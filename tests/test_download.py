...
from MIMIC_IV_MEDS.download import MockSession, download_data, MockResponse
...
def test_skip_existing_download(caplog, dataset_config):
    """
    Tests that if a file already exists and its checksum matches the one from SHA256SUMS.txt,
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        found_skip_demo = any(f"Skipping download, file already exists and valid checksum: {demo_file}" in rec.message for rec in caplog.records)
        found_skip_common = any(f"Skipping download, file already exists and valid checksum: {common_file}" in rec.message for rec in caplog.records)
...
def test_redownload_on_checksum_mismatch(caplog, dataset_config):
    """
    Tests that if a file exists but its checksum does not match the expected value,
    then the file is re-downloaded.
    """
    caplog.set_level(logging.INFO)
