...
def get_checksum_mapping(base_url: str, session: requests.Session) -> dict:
    ...
    The expected checksum file is located at <base_url>/SHA256SUMS.txt.
    Returns a dictionary mapping relative file paths to their expected SHA256 checksum.
    """
        ...
        try:
            ...
            if rel_key in mapping:
                ...
                if actual_checksum == expected_checksum:
                    ...
                    logger.info(f"Checksum mismatch for {file_path}. Expected {expected_checksum} but got {actual_checksum}. Redownloading.")
            else:
                logger.info(f"No checksum found for {rel_key} in SHA256SUMS.txt. Redownloading file: {file_path}")
        except Exception as e:
